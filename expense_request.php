...
$expenseSheets = $conn->query("SELECT er.id, p.project_name FROM expense_requests er JOIN projects p ON p.id = er.project_id WHERE er.user_id = $user_id")->fetch_all(MYSQLI_ASSOC);
...
$requests = $conn->query("
  SELECT r.*, p.project_name, u1.name as approver1, u2.name as approver2
  FROM expense_user_requests r
  JOIN projects p ON p.id = r.project_id
  LEFT JOIN users u1 ON u1.id = r.approver1_id
  LEFT JOIN users u2 ON u2.id = r.approver2_id
    ...
    <table>
      <thead>
        <tr>
          ...
          <th>Approver 1</th><th>Approver 2</th><th>Status</th>
        </tr>
      </thead>
      <tbody>
        ...
        <?php endforeach; else: ?>
          <tr><td colspan="8">No requests submitted.</td></tr>
        <?php endif; ?>
      </tbody>
    </table>
  </div>
